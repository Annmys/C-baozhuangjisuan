namespace 包装计算
{
...

    {
        ...
        new 新包装资料
            {
                ...
                装箱产品类型 = new List<string>
                {
                    "F15、单层注塑底部出线、双层注塑底部出线、（多条或短条包装）",
                    "F21、单层注塑底部出线、双层注塑底部出线、（多条或短条包装）"
                },
                包材名称 = new List<string>
                {
                    "天盒", "地盒", "加强平卡", "圆垫板", "FOF热缩袋",
        ...
        new 新包装资料
        {
            ...
            装箱产品型号 = new List<string>
            {
                "F15", "F21", "F2008", "F2010", "F2012", "F2219", "F2222", "F22", "F10", "W3525"
            },
            装箱产品类型 = new List<string>
            {
                ...
                "W3525、单层注塑正弯端部出线、单层注塑正弯底部出线、（多条或短条包装）"
            },
            包材名称 = new List<string>
            {
                "天盒", "地盒", "加强平卡", "圆垫板",
        ...
        public 新包装()
        {
            ...
            foreach (var 包装 in 装箱产品列表)
            {
                ...
                if (包装.包装名称 == "圆盘式包装470mm圆盘-07")               //圆盘式包装470mm圆盘-07
                {
                    包装.产品盘灯对应表["F15"] = new 产品盘灯信息
                    {
                        产品型号 = "F15",
                ...
                if (包装.包装名称 == "圆盘式包装600mm圆盘-07")               //圆盘式包装600mm圆盘-07
                {
                    包装.产品盘灯对应表["F15"] = new 产品盘灯信息
                    {
                        产品型号 = "F15",
        ...
        private 变量 变量 = new 变量(); // 确保这是一个实例字段
        private 匹配信息 匹配信息; // 声明匹配信息字段

        public 新包装资料 选择最佳包装(List<新包装资料> 候选包装列表, string 灯带型号, string 出线方式, int 数据条数)
        {
            // 定义权重计算函数
            int 计算包装分数(新包装资料 包装)
            {
                ...
                // 1. 匹配灯带型号（最高优先级）
                if (包装.装箱产品型号.Contains(灯带型号))
                {
                    分数 += 100; // 型号匹配得100分
                }

                // 2. 匹配出线方式
                if (包装.装箱产品类型.Any(t => t.Contains(出线方式)))
                {
                    分数 += 50; // 出线方式匹配得50分
                }

                // 3. 包装容量得分（最大20分）
                double 容量得分 = (包装.总有效面积 / 候选包装列表.Max(x => x.总有效面积)) * 20;
                分数 += (int)容量得分;

                // 4. 多条灯带专用包装加分
                if (数据条数 > 1 && 包装.是多条短条专用包装)
                {
                    分数 += 30; // 多条专用包装得30分
                }
            ...
            // 计算所有候选包装的得分
            var 包装得分列表 = 候选包装列表
                .Select(包装 => new { 包装, 分数 = 计算包装分数(包装) })
                .OrderByDescending(x => x.分数)
                .ToList();

            // 获取得分最高的包装
            var 最佳包装 = 包装得分列表.FirstOrDefault()?.包装;
            ...
            if (最佳包装 != null)
            {
                string 消息 = $"选中的包装信息：\n\n" +
                             $"包装名称: {最佳包装.包装名称}\n" +
                             $"半成品BOM物料码: {最佳包装.半成品BOM物料码}\n" +
                             $"总有效面积: {最佳包装.总有效面积}\n" +
                             $"允许多条包装: {最佳包装.允许多条包装}\n" +
                             $"是多条专用包装: {最佳包装.是多条短条专用包装}\n" +
                             $"得分: {包装得分列表.First().分数}分";

                //MessageBox.Show(消息, "选中包装信息", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("未找到合适的包装", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        ...
        //旧的选择方案
        public 新包装资料? 选择最佳包装1(List<新包装资料> 候选包装列表, int 数据条数)
        {
            string 调试信息 = "";
